...
IF NOT EXISTS (SELECT * FROM sys.types WHERE name = N'typMoeglicheAktionen' )
	CREATE TYPE typMoeglicheAktionen
		AS TABLE ( 
			...
			, [FigurBuchstabe]				CHAR(1)						NOT NULL		-- (B)auer, (L)äufer, (S)pringer, (T)urm, (K)önig, (D)ame
...
CREATE TABLE [Infrastruktur].[Figuren](
	...
	, [FigurWert]				INTEGER			NOT NULL					-- Standard: Bauer=1, Laeufer=3, Springer=3, Turm=5, Dame=10
																			-- der König ist unbezahlbar. Hier koennen taktisch ein paar
																			-- Stellschrauben gesetzt werden.
	, [FigurInternational]		CHAR(1)			NOT NULL					-- Bauer = (P)awn, Läufer = (B)ishop, Springer = K(N)ight
																			-- Turm = (R)ook, König = (K)ing, Dame = (Q)ueen
	,
...
CREATE TABLE [Infrastruktur].[Spielbrett](
	...
	, [IstSpielerWeiss]			BIT			NULL							-- 1 = TRUE
	, [FigurBuchstabe]			CHAR(1)		NULL							-- (L)äufer, (S)pringer, (T)urm, (K)önig, (D)ame, (B)auer, leer = NULL
