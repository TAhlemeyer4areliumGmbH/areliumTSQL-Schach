...
-- ###########################################################################################
-- ### Erstellung der Prozedur [Spiel].[prcTSQLziehtWeiss]                                 ###
-- ### ----------------------------------------------------------------------------------- ###
-- ### Dieses Skript erstellt oder aendert eine Prozedur, die je nach eingestelltem        ###
-- ### Schwierigkeitsgrad einen Zug mit den WEISSEN Steinen ausfuehrt.                     ### 
-- ### ----------------------------------------------------------------------------------- ###
-- ### Sicherheitshinweis:                                                                 ###
...
-- Erstellung der Prozedur [Spiel].[prcTSQLziehtWeiss]
-- -----------------------------------------------------------------------------------------
CREATE OR ALTER PROCEDURE [Spiel].[prcTSQLziehtWeiss] 
AS
BEGIN
	...
	IF (SELECT [SpielstaerkeID] FROM [Spiel].[Konfiguration] WHERE [IstSpielerWeiss] = 'TRUE') = 2				-- zufaelliger Zug
	BEGIN
...
DECLARE @Skript		VARCHAR(100)	= '531 - Prozedur [Spiel].[prcTSQLziehtWeiss] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
