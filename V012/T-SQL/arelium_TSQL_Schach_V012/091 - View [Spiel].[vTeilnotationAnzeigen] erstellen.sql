...
-- ###########################################################################################
-- ### VIEW [Spiel].[TeilnotationAnzeigen] erstellen                                       ###
-- ### ----------------------------------------------------------------------------------- ###
...
CREATE OR ALTER VIEW [Spiel].[vTeilnotationAnzeigen]
AS
	SELECT TOP 100 PERCENT
		  ROW_NUMBER() OVER(ORDER BY [INNEN].[VollzugID] ASC)			AS [OrderID]
		, CASE WHEN [INNEN].[VollzugID] = ((SELECT MAX([VollzugID]) FROM [Spiel].[Notation]) - 8)
			THEN '...'
	...
	FROM
		(
			...
			LEFT JOIN (SELECT * FROM [Spiel].[Notation]) AS [SCHWARZ]
				ON 1 = 1
...
DECLARE @Skript		VARCHAR(100)	= '091 - View [Spiel].[vTeilnotationAnzeigen] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
...
SELECT * FROM [Spiel].[vTeilnotationAnzeigen]
