...
-- -----------------------------------------------------------------------------------------
-- Erstellung der Prozedur [Spiel].[prcTSQL_vs_TSQL]
-- -----------------------------------------------------------------------------------------
CREATE OR ALTER PROCEDURE [Spiel].[prcTSQL_vs_TSQL] 
	(
...
BEGIN
	...
	DECLARE @IstSpielerWeissComputerzug			AS BIT

	WHILE NOT EXISTS (
		SELECT * FROM [Spiel].[Notation]
		WHERE	[LangeNotation] = '1-0'
			OR	[LangeNotation] = '0-1'
			OR	[LangeNotation] = '½-½'
					)
	BEGIN
		-- nun reagiert der Rechner als Gegenspieler
		DELETE FROM  @Computerzug
		...
		SET @IstSpielerWeissComputerzug		= (SELECT TOP 1 [IstSpielerWeiss] FROM @Computerzug)
...
DECLARE @Skript		VARCHAR(100)	= '522 - Prozedur [Spiel].[prcTSQL_vs_TSQL] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
