...
SELECT 
	...
	, CASE [SichtWeiss].[OrderNr]
		WHEN 1 THEN 'WEISS ' 
			+ CASE (SELECT [SpielstaerkeID] FROM [Spiel].[Konfiguration] WHERE [IstSpielerWeiss] = 'TRUE') WHEN 1 THEN '(Comp)' ELSE ''
			+ ':'
			END
		WHEN 2 THEN 'Restzeit:'
		WHEN 4 THEN 'Zugrecht/-pflicht:'
		WHEN 6 THEN 'Bewertung:'
		WHEN 8 THEN 'SCHWARZ'
			+ CASE (SELECT [SpielstaerkeID] FROM [Spiel].[Konfiguration] WHERE [IstSpielerWeiss] = 'FALSE') WHEN 1 THEN '(Comp)' ELSE ''
			+ ':'
			END
	...
	, '   '												AS [;]
	, ISNULL([MA].[LangeNotation], '')					AS [Zugideen]
	, '   '												AS [:]
	...
	LEFT JOIN 
		( 
			SELECT 
				  [ID]																AS [ID]
				, [Label]															AS [Label]
				, ISNULL(CONVERT(VARCHAR(10), [Weiss])		, 'unbewertet')			AS [Weiss]
				, ISNULL(CONVERT(VARCHAR(10), [Schwarz])	, 'unbewertet')			AS [Schwarz]
...
DECLARE @Skript		VARCHAR(100)	= '071 - Sicht [Infrastruktur].[vSpielbrett] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
