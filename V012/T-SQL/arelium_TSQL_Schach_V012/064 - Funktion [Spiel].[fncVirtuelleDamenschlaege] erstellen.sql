...
-- ###########################################################################################
-- ### Erstellung der Funktion [Spiel].[fncVirtuelleDamenschlaege]                         ###
-- ### ----------------------------------------------------------------------------------- ###
-- ### Im Schach gibt es die Fragestellung, ob eine bestimmte Figur (hier die Dame) in     ###
...
CREATE OR ALTER FUNCTION [Spiel].[fncVirtuelleDamenschlaege]
(
		...
		AND 
			(
				(
					...
					AND
					NOT EXISTS (
									SELECT [TA].[TheoretischeAktionenID] 
									...
									WHERE 1 = 1
										...
										AND [TA].[ZielReihe]		<  [MZU].[ZielReihe]
										AND [SPB].[FigurUTF8]		<> 160
								)
				)
				...
				(
					...
					NOT EXISTS (
									...
									WHERE 1 = 1
										...
										AND [TA].[FigurName]		=  'Dame'
										AND [TA].[IstSpielerWeiss]	=  @IstSpielerWeiss
										AND [TA].[ZielSpalte]		<  [MZU].[ZielSpalte]
										AND [SPB].[FigurUTF8]		<> 160
								)
				)
...
DECLARE @Skript		VARCHAR(100)	= '064 - Funktion [Spiel].[fncVirtuelleDamenschlaege] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
...
GO
/*
-- Test der Funktion [Spiel].[fncVirtuelleDamenschlaege]
...
SELECT * FROM [Spiel].[fncVirtuelleDamenschlaege](
	'TRUE'
	, @ASpielbrett
	, @AktivesFeld
	, 1)
