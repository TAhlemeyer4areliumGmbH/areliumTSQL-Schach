...
-- ###########################################################################################
-- ### VIEW [Bibliothek].[vSchonmalGespielt]                                               ###
-- ### ----------------------------------------------------------------------------------- ###
...
AS (
	...
	*
		...
		AND [PMD].[KurzeNotation] LIKE
			(
				...
				(
					...
					LEFT JOIN [AktuellesSpiel].[Notation] AS [SCHWARZ]
	...
	SELECT 1 AS [ID],	CASE (SELECT COUNT([PartiemetadatenID]) FROM [CTEBucheintrag])
							...
							ELSE 
								CASE
									WHEN (SELECT COUNT(*) FROM [AktuellesSpiel].[Notation]) > 0 THEN 'diese Zugfolge steht nicht in der Bibliothek'
	...
	SELECT 7, CASE (
				...
				INNER JOIN [AktuellesSpiel].[Konfiguration] AS [KON] ON [SST].[SpielstaerkeID] = [KON].[SpielstaerkeID]
				WHERE [KON].[IstSpielerWeiss] = [AktuellesSpiel].[fncIstWeissAmZug]()
				)
				WHEN 1 THEN 
		...
		SELECT [Bibliothek].[fncCharIndexNG2](
						' '
						, [KurzeNotation]
						, 2 * (SELECT MAX([VollzugID]) FROM [AktuellesSpiel].[Notation])	- 1
					) FROM [CTEBucheintrag]
		)
, 
	(
		(SELECT [Bibliothek].[fncCharIndexNG2](
						' '
						, [KurzeNotation]
						, 4 + 2 * (SELECT MAX([VollzugID]) FROM [AktuellesSpiel].[Notation])	
		) FROM [CTEBucheintrag])
		-
		(SELECT [Bibliothek].[fncCharIndexNG2](
						' '
