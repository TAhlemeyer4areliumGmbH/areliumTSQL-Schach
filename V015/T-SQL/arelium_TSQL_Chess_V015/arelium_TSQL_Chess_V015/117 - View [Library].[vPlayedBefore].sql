...
-- ###########################################################################################
-- ### VIEW [Library].[vPlayedBefore]                                                      ###
-- ### ----------------------------------------------------------------------------------- ###
-- ### This view randomly selects from the opening library of grandmaster games one that   ###
-- ### has started identically to the current game progression.                            ###
-- ### ----------------------------------------------------------------------------------- ###
-- ### Security note:                                                                      ###
...
-- Construction work -----------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------
CREATE OR ALTER VIEW [Library].[vPlayedBefore]
AS
...
AS (
	SELECT TOP 1
	*
	FROM [Library].[GameMetadata] AS [PMD]
	WHERE 1 = 1
		AND [PMD].[ShortNotation]LIKE
			(
				SELECT
					STRING_AGG([Move], ' ') + '%'
				FROM
				(
					SELECT TOP 100  PERCENT
						  [White].[MoveID]
						, [White].[ShortNotationSimple]
						, CONVERT(VARCHAR(3), [White].[MoveID]) + '.' + [White].[ShortNotationSimple]
						  + ' ' + ISNULL([Black].[ShortNotationSimple], '') AS [Move]
					FROM [CurrentGame].[Notation] AS [White]
					LEFT JOIN [CurrentGame].[Notation] AS [Black]
						ON [White].[MoveID]				= [Black].[MoveID]
						AND [Black].[IsPlayerWhite]		= 'FALSE'
					WHERE 1 = 1
						AND [White].[IsPlayerWhite]		= 'TRUE'
					ORDER BY [White].[MoveID] ASC
				) AS [Template]
			)
	ORDER BY NEWID()
	)

	SELECT 1 AS [ID],	CASE (SELECT COUNT([GameMetadataID]) FROM [CTEBook])
							WHEN 1 THEN
								(SELECT 'Game [' + CONVERT(VARCHAR(8), [GameMetadataID]) + '] was already played in the year ' + ISNULL(LEFT([Date], 4), '????') FROM [CTEBook])
							ELSE 
								CASE
									WHEN (SELECT COUNT(*) FROM [CurrentGame].[Notation]) > 0 THEN 'this move sequence is not in the library'
									ELSE 'Starting line-up'
								END
						END AS [Value] 
	UNION
	SELECT 2, ' by ' + [White] + ' (white' + ISNULL(CONVERT(VARCHAR(6), [EloValueWhite]), '') +  ')' FROM [CTEBook] 
	UNION
	SELECT 3, ' and ' + [Black] + ' (black' + ISNULL(CONVERT(VARCHAR(6), [EloValueBlack]), '') +  ')' FROM [CTEBook]
	UNION
	SELECT 4, ' result: ' + [Result] FROM [CTEBook]
	UNION
	SELECT 5, ' '
	UNION
	SELECt 6, 'Grandmaster continuation:'
	UNION
	SELECT 7, CASE (
				SELECT [IsGrandmasterSupportVisible] 
				FROM [Infrastructure].[Level] AS [SST]
				INNER JOIN [CurrentGame].[Configuration] AS [KON] ON [SST].[LevelID] = [KON].[LevelID]
				WHERE [KON].[IsPlayerWhite] = [CurrentGame].[fncIsNextMoveWhite]()
				)
				WHEN 1 THEN 
					(SELECT SUBSTRING([ShortNotation]
					, (
						SELECT [Library].[fncCharIndexNG2](
						' '
						, [ShortNotation]
						, 2 * (SELECT MAX([MoveID]) FROM [CurrentGame].[Notation])	- 1
					) FROM [CTEBook]
		)
, 
	(
		(SELECT [Library].[fncCharIndexNG2](
						' '
						, [ShortNotation]
						, 4 + 2 * (SELECT MAX([MoveID]) FROM [CurrentGame].[Notation])	
		) FROM [CTEBook])
		-
		(SELECT [Library].[fncCharIndexNG2](
						' '
...
DECLARE @Script		VARCHAR(100)	= '117 - View [Library].[vPlayedBefore].sql'
PRINT ' '
PRINT 'Script     :   ' + @Script
PRINT 'End        :   ' + @End
PRINT 'Time       :   ' + @Time
...
SELECT * FROM [Library].[vPlayedBefore]
GO
