...
-- ###########################################################################################
-- ### VIEW [Bibliothek].[vEroeffnungsbibliothek]                                          ###
-- ### ----------------------------------------------------------------------------------- ###
-- ### Dieses Skript erstellt oder aendert eine Sicht, die zur aktuellen Partie in der     ###
-- ### Eroeffnungsbibliothek nachschaut und zufaellig eine Partie mit identischen          ###
-- ### Anfangszuegen heraussucht.                                                          ###
-- ### ----------------------------------------------------------------------------------- ###
-- ### Sicherheitshinweis:                                                                 ###
...
CREATE OR ALTER VIEW [Bibliothek].[vEroeffnungsbibliothek]
AS
	SELECT TOP 3
		  [Seite]
		, [Veranstaltungsdatum]
	...
	WHERE 1 = 1
		AND [PartiemetadatenID] IN (SELECT DISTINCT [PartiemetadatenID] FROM [Bibliothek].[aktuelleNachschlageoptionen])
		AND 
			CASE [AktuellesSpiel].[fncIstWeissAmZug]()
				WHEN 'TRUE' THEN '1-0'
				ELSE '0-1'
			END = [Ergebnis] 
	ORDER BY NEWID()
...
DECLARE @Skript		VARCHAR(100)	= '114 - View [Bibliothek].[vEroeffnungsbibliothek] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
