...
-- ###########################################################################################
-- ### Erstellung der Funktion [AktuellesSpiel].[fncMoeglicheSpringeraktionen]             ###
-- ### ----------------------------------------------------------------------------------- ###
-- ### Moechte man herausfinden, welche Moeglichkeiten man in einer bestimmten Stellung    ###
-- ### hat die Figur "Springer" zu ziehen, bemueht man diese Funktion. Sie erwartet die    ###
...
-- Aufbauarbeiten -----------
-----------------------------
CREATE OR ALTER FUNCTION [AktuellesSpiel].[fncMoeglicheSpringeraktionen]
(
	   @IstSpielerWeiss		AS BIT
	 , @Bewertungsstellung	AS typStellung			READONLY
	 , @AktivesFeld			AS INTEGER
)
RETURNS @MoeglicheSpringerzuege TABLE 
	(
	...
	BEGIN
		INSERT INTO @MoeglicheSpringerzuege
		(
	...
	SELECT [MZU].[TheoretischeAktionenID]				AS [TheoretischeAktionenID]
...
DECLARE @Skript		VARCHAR(100)	= '042 - Funktion [AktuellesSpiel].[fncMoeglicheSpringeraktionen] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
...
/*
-- Test der Funktion [AktuellesSpiel].[fncMoeglicheSpringeraktionen]
...
SET @AktivesFeld = 9--(
	...
	--	AND [SB].[FigurBuchstabe] = 'S'
	--)

SELECT * FROM [AktuellesSpiel].[fncMoeglicheSpringeraktionen] (
	'True'
	, @ASpielbrett
	, @AktivesFeld)
ORDER BY 2,3,4,5,7
