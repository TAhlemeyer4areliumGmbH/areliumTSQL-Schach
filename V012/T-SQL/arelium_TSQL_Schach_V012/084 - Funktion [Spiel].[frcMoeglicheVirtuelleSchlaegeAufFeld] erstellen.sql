...
CREATE OR ALTER FUNCTION [Spiel].[frcMoeglicheVirtuelleSchlaegeAufFeld]
(
	   @IstSpielerWeiss		AS BIT
	 , @Bewertungsstellung	AS typStellung			READONLY
	 , @VirtuellesZielfeld	AS INTEGER
)
	...
	BEGIN
		...
		BEGIN 
			...
			SELECT [TheoretischeAktionenID], 1, [FigurName], [IstSpielerWeiss], [StartSpalte],
				...
				[UmwandlungsfigurBuchstabe], [ZugIstSchlag], [ZugIstKurzeRochade], [ZugIstLangeRochade],
				[ZugIstEnPassant], [LangeNotation], [KurzeNotationEinfach], [KurzeNotationKomplex] 
			FROM [Spiel].[fncMoeglicheDamenschlaege] (@IstSpielerWeiss, @Bewertungsstellung, @Damenfeld)
			WHERE [Zielfeld] = @VirtuellesZielfeld
...
DECLARE @Skript		VARCHAR(100)	= '084 - Funktion [Spiel].[frcMoeglicheVirtuelleSchlaegeAufFeld] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
...
/*
-- Test der Funktion [Spiel].[frcMoeglicheVirtuelleSchlaegeAufFeld]
...
SELECT * FROM [Spiel].[frcMoeglicheVirtuelleSchlaege]('FALSE', @Bewertungsstellung)
