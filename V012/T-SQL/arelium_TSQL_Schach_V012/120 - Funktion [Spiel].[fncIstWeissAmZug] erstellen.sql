...
-- ### ----------------------------------------------------------------------------------- ###
-- ### In der Tabelle [Spiel].[Notation] werden alle bisher in der aktuellen Partie        ###
-- ### durchgefuehrten Zuege notiert. Anhand dieser Angaben kann man eindeutig             ###
...
-- -----------------------------------------------------------------------------------------
-- Erstellung der Funktion [Spiel].[fncIstWeissAmZug]
-- -----------------------------------------------------------------------------------------
CREATE OR ALTER FUNCTION [Spiel].[fncIstWeissAmZug]
()
RETURNS BIT
AS
BEGIN
	...
	IF EXISTS (SELECT COUNT(*) AS [Anzahl] FROM [Spiel].[Notation] GROUP BY [VollzugID] HAVING COUNT(*) = 1)
	BEGIN
...
DECLARE @Skript		VARCHAR(100)	= '120 - Funktion [Spiel].[fncIstWeissAmZug] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
...
/*
-- Test der Funktion [Spiel].[fncIstWeissAmZug]
