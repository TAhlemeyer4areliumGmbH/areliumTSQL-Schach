...
RETURNS @MoeglicheAktionen TABLE 
	...
	BEGIN
		...
		-- --------------------------------------------------------------------------------------
		-- Eine Rochade ist nur moeglich, wenn der zugehoeriger Turm und der Koenig an der 
		-- "richtigen" Position stehen (also auch nicht geschlagen wurden)
		-- --------------------------------------------------------------------------------------

		-- kurze Rochade WEISS
		IF NOT EXISTS (SELECT * FROM @Bewertungsstellung WHERE [Feld] = 57 AND [FigurBuchstabe] = 'T' AND [IstSpielerWeiss] = 'TRUE')
			OR NOT EXISTS (SELECT * FROM @Bewertungsstellung WHERE [Feld] = 33 AND [FigurBuchstabe] = 'K' AND [IstSpielerWeiss] = 'TRUE')
		BEGIN
			DELETE FROM @MoeglicheAktionen WHERE [LangeNotation] = 'o-o' AND [IstSpielerWeiss] = 'TRUE'
		END

		-- kurze Rochade SCHWARZ
		IF NOT EXISTS (SELECT * FROM @Bewertungsstellung WHERE [Feld] = 64 AND [FigurBuchstabe] = 'T' AND [IstSpielerWeiss] = 'FALSE')
			OR NOT EXISTS (SELECT * FROM @Bewertungsstellung WHERE [Feld] = 40 AND [FigurBuchstabe] = 'K' AND [IstSpielerWeiss] = 'FALSE')
		BEGIN
			DELETE FROM @MoeglicheAktionen WHERE [LangeNotation] = 'o-o' AND [IstSpielerWeiss] = 'FALSE'
		END
		...
		-- lange Rochade WEISS
		IF NOT EXISTS (SELECT * FROM @Bewertungsstellung WHERE [Feld] =  1 AND [FigurBuchstabe] = 'T' AND [IstSpielerWeiss] = 'TRUE')
			OR NOT EXISTS (SELECT * FROM @Bewertungsstellung WHERE [Feld] = 33 AND [FigurBuchstabe] = 'K' AND [IstSpielerWeiss] = 'TRUE')
		BEGIN
			DELETE FROM @MoeglicheAktionen WHERE [LangeNotation] = 'o-o-o' AND [IstSpielerWeiss] = 'TRUE'
		END

		-- kurze Rochade SCHWARZ
		IF NOT EXISTS (SELECT * FROM @Bewertungsstellung WHERE [Feld] =  8 AND [FigurBuchstabe] = 'T' AND [IstSpielerWeiss] = 'FALSE')
			OR NOT EXISTS (SELECT * FROM @Bewertungsstellung WHERE [Feld] = 40 AND [FigurBuchstabe] = 'K' AND [IstSpielerWeiss] = 'FALSE')
		BEGIN
			DELETE FROM @MoeglicheAktionen WHERE [LangeNotation] = 'o-o-o' AND [IstSpielerWeiss] = 'FALSE'
		END		 


		-- --------------------------------------------------------------------------------------
		-- eine Rochade ist auch dann unmoeglich, wenn Koenig oder Turm schonmal gezogen
		-- haben - selbst wenn sie auf das Ausgangsfeld zurueckgekehrt sind. Es reicht also 
		...
		WHERE 1 = 1
			AND [LangeNotation] LIKE 'o-o%'	
			AND [IstSpielerWeiss] = 'TRUE'
			AND	(SELECT COUNT(*) FROM [Spiel].[Notation] WHERE [LangeNotation] LIKE 'K%' AND [IstSpielerWeiss] = 'TRUE') > 0

		DELETE FROM @MoeglicheAktionen
		WHERE 1 = 1
			AND [LangeNotation] LIKE 'o-o%'	
			AND [IstSpielerWeiss] = 'FALSE'
			AND	(SELECT COUNT(*) FROM [Spiel].[Notation] WHERE [LangeNotation] LIKE 'K%' AND [IstSpielerWeiss] = 'FALSE') > 0

		DELETE FROM @MoeglicheAktionen
		WHERE 1 = 1
			AND [LangeNotation] = 'o-o'	
			AND [IstSpielerWeiss] = 'TRUE'
			AND	(SELECT COUNT(*) FROM [Spiel].[Notation] WHERE [LangeNotation] LIKE 'Ta1%' AND [IstSpielerWeiss] = 'TRUE') > 0

		DELETE FROM @MoeglicheAktionen
		WHERE 1 = 1
			AND [LangeNotation] = 'o-o'	
			AND [IstSpielerWeiss] = 'FALSE'
			AND	(SELECT COUNT(*) FROM [Spiel].[Notation] WHERE [LangeNotation] LIKE 'Ta8%' AND [IstSpielerWeiss] = 'FALSE') > 0

		DELETE FROM @MoeglicheAktionen
		WHERE 1 = 1
			AND [LangeNotation] = 'o-o-o'	
			AND [IstSpielerWeiss] = 'TRUE'
			AND	(SELECT COUNT(*) FROM [Spiel].[Notation] WHERE [LangeNotation] LIKE 'Th1%' AND [IstSpielerWeiss] = 'TRUE') > 0

		DELETE FROM @MoeglicheAktionen
		WHERE 1 = 1
			AND [LangeNotation] = 'o-o-o'	
			AND [IstSpielerWeiss] = 'FALSE'
			AND	(SELECT COUNT(*) FROM [Spiel].[Notation] WHERE [LangeNotation] LIKE 'Th8%' AND [IstSpielerWeiss] = 'FALSE') > 0


		-- eine Rochade ist auch dann unmoeglich, wenn der Koenig von/ueber/auf ein Feld 
		-- ziehen muss, welches aktuell bedroht ist (=im Schach staende)
		...
		WHERE 1 = 1
			...
			AND [IstSpielerWeiss] = 'TRUE'
			AND (
					[Spiel].[fncIstFeldBedroht]([IstSpielerWeiss], @Bewertungsstellung, 41) = 'TRUE'
					OR
					[Spiel].[fncIstFeldBedroht]([IstSpielerWeiss], @Bewertungsstellung, 49) = 'TRUE'
				)
		...
		WHERE 1 = 1
			...
			AND [IstSpielerWeiss] = 'FALSE'
			AND (
					[Spiel].[fncIstFeldBedroht]([IstSpielerWeiss], @Bewertungsstellung, 48) = 'TRUE'
					OR
					[Spiel].[fncIstFeldBedroht]([IstSpielerWeiss], @Bewertungsstellung, 56) = 'TRUE'
				)
		...
		WHERE 1 = 1
			...
			AND [IstSpielerWeiss] = 'TRUE'
			AND (
					[Spiel].[fncIstFeldBedroht]([IstSpielerWeiss], @Bewertungsstellung, 17) = 'TRUE'
					OR
					[Spiel].[fncIstFeldBedroht]([IstSpielerWeiss], @Bewertungsstellung, 25) = 'TRUE'
				)
		...
		-- --------------------------------------------------------------------------------------
		-- Eine Rochade ist nur moeglich, wenn sich zwischen Turm und Koenig keine weiteren  
		-- Figuren befinden
		-- --------------------------------------------------------------------------------------
...
DECLARE @Skript		VARCHAR(100)	= '080 - Funktion [Spiel].[fncMoeglicheAktionen] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
