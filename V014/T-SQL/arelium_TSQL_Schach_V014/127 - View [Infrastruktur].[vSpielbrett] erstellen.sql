...
-- ###########################################################################################
-- ### VIEW [AktuellesSpiel].[vBrettansicht]                                               ###
-- ### ----------------------------------------------------------------------------------- ###
...
SELECT 
	  CASE 
		WHEN (SELECT COUNT(*) FROM [AktuellesSpiel].[vTeilnotationAnzeigen]) = 0 THEN '' 
	...
	, CASE [SichtWeiss].[OrderNr]
		WHEN 1 THEN N'WEISS ' 
			+ CASE (SELECT [SpielstaerkeID] FROM [AktuellesSpiel].[Konfiguration] WHERE [IstSpielerWeiss] = 'TRUE') WHEN 1 THEN '' ELSE '(Comp)'
			+ ':'
			END
		WHEN 2 THEN N'Restzeit:'
		WHEN 3 THEN N'50-Zuege-Regel:'
		WHEN 4 THEN N'Zugrecht/-pflicht:'
		WHEN 5 THEN N'en-passant-Moeglichkeit:'
		WHEN 6 THEN N'Bewertung:'
		WHEN 7 THEN N'50-Zuege-Regel:'
		WHEN 8 THEN N'SCHWARZ'
			+ CASE (SELECT [SpielstaerkeID] FROM [AktuellesSpiel].[Konfiguration] WHERE [IstSpielerWeiss] = 'FALSE') WHEN 1 THEN N'' ELSE '(Comp)'
			+ ':'
			END
	...
	END													AS [ ]
	, CASE [SichtWeiss].[OrderNr]
		WHEN 1 THEN ISNULL((SELECT [Spielername] FROM [AktuellesSpiel].[Konfiguration] WHERE [IstSpielerWeiss] = 'TRUE'), '--')
		WHEN 2 THEN ISNULL([Infrastruktur].[fncSekundenAlsUhrzeitFormatieren]((SELECT [RestzeitInSekunden] FROM [AktuellesSpiel].[Konfiguration] WHERE [IstSpielerWeiss] = 'TRUE')), '--')
		WHEN 3 THEN ISNULL((SELECT ISNULL(CONVERT(CHAR(3), [Anzahl50ZugRegel]), '--') FROM [AktuellesSpiel].[Konfiguration] WHERE [IstSpielerWeiss] = 'TRUE'), '--')
		WHEN 4 THEN CASE [AktuellesSpiel].[fncIstWeissAmZug]() WHEN 'TRUE' THEN 'WEISS' ELSE 'SCHWARZ' END
		WHEN 5 THEN ISNULL((SELECT [EnPassant] FROM [AktuellesSpiel].[Konfiguration] WHERE [IstSpielerWeiss] = (([AktuellesSpiel].[fncIstWeissAmZug]() + 1) % 2)), '')
		WHEN 6 THEN ISNULL(CONVERT(VARCHAR(8), [Statistik].[fncAktuelleStellungBewerten]()), '--')
		WHEN 7 THEN ISNULL((SELECT CONVERT(CHAR(3), [Anzahl50ZugRegel]) FROM [AktuellesSpiel].[Konfiguration] WHERE [IstSpielerWeiss] = 'FALSE'), '--')
		WHEN 8 THEN ISNULL((SELECT [Spielername] FROM [AktuellesSpiel].[Konfiguration] WHERE [IstSpielerWeiss] = 'FALSE'), '--')
		WHEN 9 THEN ISNULL([Infrastruktur].[fncSekundenAlsUhrzeitFormatieren]((SELECT [RestzeitInSekunden] FROM [AktuellesSpiel].[Konfiguration] WHERE [IstSpielerWeiss] = 'FALSE')), '--')
		ELSE ''
	END													AS [_]
	, '|'												AS [,,]
	, [SichtSchwarz].[.]								AS [,]
	...
	, '|'												AS [;]
	, CASE (SELECT [ComputerSchritteAnzeigen] FROM [AktuellesSpiel].[Konfiguration] WHERE [IstSpielerWeiss] = (([AktuellesSpiel].[fncIstWeissAmZug]() + 1) % 2)) 
...
FROM
	...
	LEFT JOIN [AktuellesSpiel].[vGeschlageneFiguren] AS [GF]
	...
	--    (hierzu muss VORHER die Prozedur [AktuellesSpiel].[prcAktionenFuerAktuelleStellungWegschreiben]
	...
	LEFT JOIN 
		(
			SELECT TOP 9 
				...
				, *
			FROM [AktuellesSpiel].[MoeglicheAktionen] 
		) AS [MA]
	...
	LEFT JOIN 
		( 
			...
			FROM [AktuellesSpiel].[vTeilnotationAnzeigen]
		) AS [LN]
