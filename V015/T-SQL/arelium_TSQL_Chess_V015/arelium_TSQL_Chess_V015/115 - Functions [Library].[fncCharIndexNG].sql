...
-- ###########################################################################################
-- ### Functions [Library].[fncCharIndexNG1] and [Library].[fncCharIndexNG2]               ###
-- ### ----------------------------------------------------------------------------------- ###
-- ### These functions determine the nth occurrence of a search pattern in a string and    ###
-- ### return its position.                                                                ###
-- ### ----------------------------------------------------------------------------------- ###
-- ### Security note:                                                                      ###
...
-- Construction work -----------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------
CREATE OR ALTER FUNCTION [Library].[fncCharIndexNG1] (
	  @SearchPattern		VARCHAR(8000)
	, @SearchedString		VARCHAR(MAX)
	, @StartPoint			INTEGER
	, @Count				TINYINT
)
RETURNS BIGINT
...
BEGIN
	...
	IF ISNULL(@Count, 0) <= 0 OR ISNULL(@StartPoint, 0) <= 0
	BEGIN
		SELECT @Pos = 0
	END 
	ELSE 
	BEGIN
		IF @Count = 1
		BEGIN
			SELECT @Pos = CHARINDEX(@SearchPattern, @SearchedString, @StartPoint)
		END 
		ELSE 
		BEGIN
			SELECT @Pos = [Library].[fncCharIndexNG1]
				(
					  @SearchPattern
					, @SearchedString
					, NULLIF(CHARINDEX(@SearchPattern, @SearchedString, @StartPoint), 0) + 1
					, @Count - 1)
		END
	END
	RETURN @Pos
...
CREATE OR ALTER FUNCTION [Library].[fncCharIndexNG2] (
	  @SearchPattern	VARCHAR(8000)
	, @SearchedString	VARCHAR(MAX)
	, @Count			TINYINT
)
...
AS
BEGIN
	RETURN [Library].[fncCharIndexNG1](@SearchPattern, @SearchedString, 1, @Count)
END
GO
...
DECLARE @Script		VARCHAR(100)	= '115 - Funktionen [Library].[fncCharIndexNG].sql'
PRINT ' '
PRINT 'Script     :   ' + @Script
PRINT 'End        :   ' + @End
PRINT 'Time       :   ' + @Time
...
SELECT [Library].[fncCharIndexNG2]('_', @val, 3)
GO
*/
