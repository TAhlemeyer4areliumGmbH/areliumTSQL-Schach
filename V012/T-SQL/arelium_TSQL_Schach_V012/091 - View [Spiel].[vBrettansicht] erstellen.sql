...
SELECT 
	...
	, CASE [SichtWeiss].[OrderNr]
		WHEN 1 THEN 'WEISS:'
		WHEN 2 THEN 'Restzeit:'
		WHEN 4 THEN 'Zugrecht/-pflicht:'
		WHEN 8 THEN 'SCHWARZ:'
		WHEN 9 THEN 'Restzeit:'
		ELSE ''
	END													AS [Schluessel]
	, CASE [SichtWeiss].[OrderNr]
		WHEN 1 THEN 'Torsten'
		WHEN 2 THEN ISNULL((SELECT CASE WHEN [RestzeitInSek] >= 86400 THEN CONVERT(VARCHAR(5),[RestzeitInSek]/86400) + ' Tage ' ELSE '' END + CONVERT(VARCHAR(8), DATEADD(SECOND, [RestzeitInSek], 0), 108) FROM [Spiel].[Zeitmessung] WHERE [IstSpielerWeiss] = 'TRUE'), 'Partie ungestartet')
		WHEN 4 THEN 'WEISS'
		WHEN 8 THEN 'Computer (T-SQL)'
		WHEN 9 THEN ISNULL((SELECT CASE WHEN [RestzeitInSek] >= 86400 THEN CONVERT(VARCHAR(5),[RestzeitInSek]/86400) + ' Tage ' ELSE '' END + CONVERT(VARCHAR(8), DATEADD(SECOND, [RestzeitInSek], 0), 108) FROM [Spiel].[Zeitmessung] WHERE [IstSpielerWeiss] = 'FALSE'), 'Partie ungestartet')
		ELSE ''
	END													AS [Wert]
	, '   '												AS [  ]
	, [SichtSchwarz].[.]								AS [.]
	...
	, [MA].[LangeNotation]								AS [LangeNotation]
FROM
	(
...
DECLARE @Skript		VARCHAR(100)	= '071 - Sicht [xxx].[vZugvorschau] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
