...
-- -----------------------------------------------------------------------------------------
-- Erstellung der Prozedur [Spiel].[prcTSQL_vs_Mensch]
-- -----------------------------------------------------------------------------------------
CREATE OR ALTER PROCEDURE [Spiel].[prcTSQL_vs_Mensch] 
	(
...
BEGIN
	...
	EXECUTE [Spiel].[prcInitialisierung] 
		...
		, @NameSchwarz
		, @SpielstaerkeWeiss
		, 1								-- fixer Wert, da SCHWARZ immer ein menschlicher Spieler ist!
	...
	IF (SELECT [SpielstaerkeID] FROM [Spiel].[Konfiguration] WHERE [IstSpielerWeiss] = 'FALSE') BETWEEN 3 AND 4				-- zufaelliger Zug
	BEGIN
...
DECLARE @Skript		VARCHAR(100)	= '521 - Prozedur [Spiel].[prcTSQL_vs_Mensch] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
