...
-- ###########################################################################################
-- ### Erstellung der Funktion [Spiel].[fncMoeglicheKoenigsschlaege]                       ###
-- ### ----------------------------------------------------------------------------------- ###
-- ### Moechte man herausfinden, welche Felder in einer bestimmten Stellung durch die      ###
-- ### Figur "Koenig" beschuetzt wreden, bemueht man diese Funktion. Sie erwartet die      ###
...
-- ###                                                                                     ###
-- ### Vorsicht:                                                                           ###
-- ### Die Figur "Koenig" hat laut Speilregeln einen Sonderstatus. Sie darf sich naemlich  ###
-- ### nicht aktiv selber in ein Schach bewegen (also auch keine geschuetzte Figur         ###
-- ### schlagen!)                                                                          ###
-- ###                                                                                     ###
...
CREATE OR ALTER FUNCTION [Spiel].[fncMoeglicheKoenigsschlaege]
(
...
RETURNS @MoeglicheKoenigsschlaege TABLE 
	...
	WHERE 1 = 1
		...
		AND 
			(
				[SPB].[FigurUTF8]				= 160  -- leeres Feld
				OR
				[SPB].[IstSpielerWeiss]			<> @IstSpielerWeiss
			)
		AND [Spiel].[fncIstFeldBedroht](@IstSpielerWeiss, @Bewertungsstellung, [SPB].[Feld]) = 'FALSE'
	RETURN
	END
GO			
...
DECLARE @Skript		VARCHAR(100)	= '065 - Funktion [Spiel].[fncMoeglicheKoenigsschlaege] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
...
/*
-- Test der Funktion [Spiel].[fncMoeglicheKoenigsschlaege]
...
SELECT * FROM [Spiel].[fncMoeglicheKoenigsschlaege] (
	'True'
	, @ASpielbrett
	, @AktivesFeld)
ORDER BY 2,3,4,5,7
