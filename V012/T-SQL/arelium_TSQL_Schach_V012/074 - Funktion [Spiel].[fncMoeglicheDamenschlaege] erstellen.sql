...
-- ###########################################################################################
-- ### Erstellung der Funktion [Spiel].[fncMoeglicheDamenschlaege]                         ###
-- ### ----------------------------------------------------------------------------------- ###
-- ### Moechte man herausfinden, welche Moeglichkeiten man in einer bestimmten Stellung    ###
-- ### hat mit der Figur "Dame" zu schlagen, bemueht man diese Funktion. Sie erwartet die  ###
-- ### Angabe einer Stellung (dies muss nicht zwingend [Infrastruktur].[Spielbrett] sein!) ###
-- ### und des Feldes, fuer den die Schlagmoeglichkeiten abgefragt werden sollen. So       ###
-- ### sind Aussagen zu beliebigen Stellungen denkbar.                                     ###
-- ###                                                                                     ###
...
CREATE OR ALTER FUNCTION [Spiel].[fncMoeglicheDamenschlaege]
(
	   @IstSpielerWeiss		AS BIT
	 , @Bewertungsstellung	AS typStellung			READONLY
	 , @AktivesFeld			AS INTEGER
)
RETURNS @MoeglicheDamenschlaege TABLE 
	(
	...
	BEGIN
		INSERT INTO @MoeglicheDamenschlaege
		(
					...
					(
						...
						WHERE 1 = 1
							...
							AND [TA].[ZielReihe]		<=
								ISNULL((
									SELECT MIN(ZielReihe)
									FROM [Infrastruktur].[TheoretischeAktionen]		AS [TAI]
									LEFT JOIN @Bewertungsstellung					AS [ISB]
										ON [TAI].[ZielFeld] = [ISB].[Feld]
									WHERE 1 = 1
										AND [TAI].[Richtung]		= 'RO'
										AND [TAI].[StartFeld]		=  [MZU].[StartFeld]
										AND [TAI].[FigurName]		= 'Dame'
										AND [TAI].[IstSpielerWeiss]	= @IstSpielerWeiss
										AND [ISB].[FigurUTF8]		<> 160
										), 8)
							AND [TA].[ZugIstSchlag]		= CASE 
																WHEN (
...
DECLARE @Skript		VARCHAR(100)	= '074 - Funktion [Spiel].[fncMoeglicheDamenschlaege] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
...
GO
/*
-- Test der Funktion [Spiel].[fncMoeglicheDamenschlaege]
...
SELECT * FROM [Spiel].[fncMoeglicheDamenschlaege] (
	'TRUE'
	, @ASpielbrett
	, @AktivesFeld)
ORDER BY 2,3,4,5,7
