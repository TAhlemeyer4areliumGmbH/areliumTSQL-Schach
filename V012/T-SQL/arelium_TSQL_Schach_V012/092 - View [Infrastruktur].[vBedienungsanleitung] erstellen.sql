...
SELECT 2, ''
UNION
SELECT 3, CONVERT(NVARCHAR(500), 'eine Aktion (egal ob Zug oder Schlag) bedarf nur der Angabe der Start- und der Zielkoordinate. Handelt es sich um eine Bauerumwandlung, ist anzugeben welche Figur eingetauscht')
UNION
SELECT 4, CONVERT(NVARCHAR(500), 'wurde (''L'', ''S'', ''T'', ''D'') - sonst ist der dritte Parameter NULL. Fällt der Zug unter die <en passant>-Regel, ist der vierte Parameter mit ''TRUE'' anzugeben (sonst ''FALSE'').')
UNION
SELECT 5, CONVERT(NVARCHAR(500), 'Der abschliessende Parameter beantwortet die Frage, ob Du die WEISSEN oder die SCHWARZEN Steine spielst und ist daher auf ' + CASE [Spiel].[fncIstWeissAmZug]() WHEN 'TRUE' THEN 'WEISS' ELSE 'SCHWARZ' END + ' zu setzen')
UNION
SELECT 6, CONVERT(NVARCHAR(500), 'Bei einer Rochade ist nur die Königsbewegung auszuführen. Aufgeben kannst Du über den Befehl <EXECUTE [Spiel].[prcAktuellesSpielAufgeben]>. Ein Remis bietet man mit <EXECUTE [Spiel].[prcRemisAnbieten]> an.')
UNION
SELECT 7, CONVERT(NVARCHAR(500), '')
UNION
SELECT 8, CONVERT(NVARCHAR(500), 'Beispiel für einen korrekten Aufruf (die Standarderöffnung mit dem Doppelschritt des Damenbauern):')
UNION
SELECT 9, CONVERT(NVARCHAR(500), 'EXECUTE [Spiel].[prcZugAusfuehren] ''d2'', ''d4'', NULL, ''FALSE'', ''TRUE''')
GO
...
DECLARE @Skript		VARCHAR(100)	= '092 - Sicht [Infrastruktur].[vSpielbrett] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
