...
-- ###########################################################################################
-- ### Erstellung der Funktion [Spiel].[fncMoeglicheBauernaktionen]		                   ###
-- ### ----------------------------------------------------------------------------------- ###
...
CREATE OR ALTER FUNCTION [Spiel].[fncMoeglicheBauernaktionen]
(
	...
	WHERE 1 = 1
		...
		AND (
				(
					...
					AND (EXISTS (SELECT * FROM [Spiel].[Notation] AS [Innen] WHERE 1 = 1
									...
									AND [Innen].[VollzugID]							= (SELECT MAX([VollzugID]) FROM [Spiel].[Notation] WHERE [IstSpielerWeiss] = ((@IstSpielerWeiss + 1) % 2))
								)
						)
				)
				OR
...
DECLARE @Skript		VARCHAR(100)	= '046 - Funktion [Spiel].[fncMoeglicheBauernaktionen] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
...
SELECT * FROM [Spiel].[fncMoeglicheBauernaktionen] (
	'True'
	, @ASpielbrett
	, @AktivesFeld)
ORDER BY 2,3,4,5,7
