...
-- ###########################################################################################
-- ### Erstellung der Funktion [Spiel].[fncMoeglicheTurmaktionen]                          ###
-- ### ----------------------------------------------------------------------------------- ###
-- ### Moechte man herausfinden, welche Moeglichkeiten man in einer bestimmten Stellung    ###
-- ### hat die Figur "Turm" zu ziehen, bemueht man diese Funktion. Sie erwartet die Angabe ###
-- ### einer Stellung (dies muss nicht zwingend [Infrastruktur].[Spielbrett] sein!) und    ###
-- ### des Feldes, fuer den die Bewegungs- und Schlagmoeglichkeiten abgefragt werden       ###
...
CREATE OR ALTER FUNCTION [AktuellesSpiel].[fncMoeglicheTurmaktionen]
(
...
RETURNS @MoeglicheTurmaktionen TABLE 
	(
	...
	BEGIN
		INSERT INTO @MoeglicheTurmaktionen
		(
		...
		WHERE 1 = 1
			...
			AND [SPB].[FigurUTF8] NOT IN 
				(
					SELECT [FigurUTF8] FROM [Infrastruktur].[Figuren]
					WHERE 1 = 1
...
DECLARE @Skript		VARCHAR(100)	= '041 - Funktion [Spiel].[fncMoeglicheTurmaktionen] erstellen.sql'
PRINT ' '
PRINT 'Skript     :   ' + @Skript
PRINT 'Ende       :   ' + @Ende
PRINT 'Zeit       :   ' + @Zeit
...
/*
-- Test der Funktion [AktuellesSpiel].[fncMoeglicheTurmaktionen]
...
SELECT * FROM [aktuellesSpiel].[fncMoeglicheTurmaktionen] (
	'True'
	, @ASpielbrett
	, @AktivesFeld)
ORDER BY 2,3,4,5,7
